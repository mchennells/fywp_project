...
# Note that not every student will have accepted an offer hence why the need to merge these with student records to get the picture of who arrived
...
raw_data_full <- read_csv(file = path_data)
...
# The original data set used is "WBS_TK_Extract_20201208_amended WP flags for FY only_anoymised.xlsx" but this was missing data for most recent FY cohort, so used most up to date Johnstone data
d_johnstone_FYonly <- read_excel(path_johnstone)
...
wp_check <- d_wp_final %>% 
  count(FY_startyear, wp_sum_with_awards)

  d_wp_final %>% count(wp_sum_with_awards)

plot <- ggplot(
  wp_check, aes(x=wp_sum_with_awards, y=n, fill = FY_startyear)) +
  stat_summary(fun = mean, geom='bar', width=0.5, position=position_dodge(0.5))
plot
# See that very few missing, and most that are come from the first 2015/16 cohort

write.xlsx(d_wp_final,'output/d_wp_FY_final_220822.xlsx',rowNames = F) 
write_csv(d_wp_final,'output/d_wp_FY_final_220822.csv')
...
d_wp_final_plusJ <- d_wp_four %>% 
  arrange(wp_sum)
write.xlsx(d_wp_final_plusJ,'output/d_wp_FY_final_plusJ_220822.xlsx',rowNames = F) 
write_csv(d_wp_final_plusJ,'output/d_wp_FY_final_plusJ_220822.csv')
# ------------------------------------------------------------------------------------------------------------
